...
Name = 'my103';                               % 読み込むBinファイルが入ったフォルダ名を入力
...
% 中心波長から中心周波数を算出
CenterWavelength = 1532.04830;                       % 中心波長 [nm]
Fc = 299792458 / (CenterWavelength * 1e-9);     % 中心光周波数 [Hz]

% x軸の生成 (自己研究と合致しているか確認すること)
N = 2^25;                                    % サンプル数 [S]
Fs = 1.8e9;                                  % サンプリングレート [S/s] 
t = (0:N-1) / Fs;                            % 時間軸の生成
...
if Judge == 1 || Judge == 2
    ...
    width = length(num2str(length(CoeffB)));        % 係数pの桁幅を記録
    
    % 近似関数の各係数をtxt形式で保存
    fprintf(fid, '         Modified Conv. Method    Prop. Method\n\n');
    for i = 1:length(CoeffB)
        fprintf(fid, sprintf('p%%%dd =  %%10.6f\n', width), i, CoeffB(i));
    end
    fprintf(fid, '\n');
...
if Judge == 1 || Judge == 2
    ...
    fprintf(fid, 'Opt Peak Judge2: %.2f \n', OptPeakJudge2);
    fprintf(fid, '\n');
