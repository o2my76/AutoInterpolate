...
Name = 'my54';                               % 読み込むBinファイルが入ったフォルダ名を入力
Judge = 0;                                   % データ保存の有無 (No: 0, Yes: 1, Yes with txt: 2)
k = 101;        % 変化させる倍率の数
m = 50;         % 倍率を変化させる際の増分 (例: 50と入力すると、算出値の倍率が50ずつ変化する)

% 波長計で取得した時間を入力 [s] (適宜変更すること)
AcquisitionMin = 00;                          % データ取得時間 [分]
AcquisitionSec = 39;                         % データ取得時間 [秒]
...
Amp = zeros(1, k);     % 倍率を変化させながら、RF領域から光領域への変換を実行する際の倍率を保存する配列の初期化
...
disp('Peak Residual Between HITRAN and Proposed Method [GHz]'); disp(Amp/1e9);      % 倍率を変化させたときの算出値のピーク位置の差分の標準偏差を表示

%% 各jにおけるAmpの値をtxt形式で保存
j_shift = ((1:k) - (k - 1) / 2).';   % (j - (k - 1) / 2)
A_Prop_Save = A_Prop_List(:);        % A_Propを縦ベクトルにする
Amp_GHz = Amp(:) / 1e9;              % AmpをGHz単位に変換

SaveData = [j_shift, A_Prop_Save, Amp_GHz];

txtFileName = fullfile(NewFolder, [Name, 'Amp_Results.txt']);

fid = fopen(txtFileName, 'w');
fprintf(fid, 'j_shift\tA_Prop\tAmp_GHz\n');
